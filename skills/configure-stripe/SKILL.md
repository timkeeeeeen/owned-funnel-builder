...
description: Configure, test, or troubleshoot Stripe for this funnel system, including hosted Checkout, product and Price registration, D1 provider mappings, card saving, off-session one-click upsells, hosted SCA fallback, signed webhooks, Postmark fulfillment, Admaxxer attribution, and duplicate-charge protection. Use for Stripe setup or any change to Stripe prices, checkout sessions, PaymentIntents, webhooks, upsell decisions, or fulfillment.
---
...
3. Confirm the exact test or live mode, product names, one-time prices, currencies, return paths, bump default, upsell order, refund policy, and real customer-access URLs. Require Postmark because Stripe has no native digital-file entitlement.
...
10. Verify the raw webhook body, timestamp tolerance, mode, and Stripe signature before changing state. Deduplicate events durably, fulfill through Postmark, and send Admaxxer revenue only from verified payment success.
11. Run function tests, type checks, a Workers Functions build, and local D1 migrations. Test invalid signatures, stale signatures, repeated accepts, fallback checkout, duplicate events, and missing Postmark without making a live charge.
12. Run `$audit-funnel` before publishing. A live paid test requires explicit approval of the amount and refund handling.

## Handoff

Tell the owner which mode is connected, whether all Products and Prices are mapped, whether the webhook and Postmark are verified, and which no-charge tests passed. Do not expose IDs or secrets unless an exact non-secret identifier is necessary for troubleshooting.
