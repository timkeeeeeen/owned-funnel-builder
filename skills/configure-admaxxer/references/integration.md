...
## Dodo and Payments API contract
...
The funnel must verify Dodo's signature before trusting the payment. After fulfillment, its server sends the successful payment to:
...
The JSON body uses the Dodo `payment_id` as `transaction_id`, converts `total_amount` from the currency's smallest unit, and includes `admaxxer_visitor_id` and customer email when available. The transaction ID makes retries idempotent. Keep the Admaxxer API key in a private Cloudflare secret, never in public Astro variables or browser code.
...
- Dodo sends a signed `payment.succeeded` event to the funnel's webhook.
