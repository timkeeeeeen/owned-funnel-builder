...
3. Run the repository's format, lint, type, KPI, build, and Functions checks that exist. Inspect their scope before relying on them.
...
8. Exercise checkout with the bump declined and selected at a no-charge level. Verify upsell decline paths and structurally verify accept paths without an unauthorized charge.
9. Verify fulfillment idempotency and entitlements through sandbox or safe fixtures.
10. Repeat relevant checks against the stable production URL and exact commit.
11. Fix in-scope defects, rerun affected checks, and report residual limitations precisely.
