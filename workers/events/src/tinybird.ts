...
export async function sendTinybird(event: CanonicalEvent, env: EventsEnv, privacySubjectId?: string | null): Promise<DeliveryResult> {
  ...
  const subject = privacySubjectId ?? event.identity.visitor_id ?? event.identity.person_id ?? '';
